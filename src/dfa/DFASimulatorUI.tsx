...
import { Play, StepForward, RotateCcw, AlertCircle, CheckCircle2, Info, Terminal, ChevronRight } from 'lucide-react';
...
export default function DFASimulatorUI() {
  ...
  const [selectedDfaKey, setSelectedDfaKey] = useState<keyof typeof DFA_EXAMPLES>('dfa1');
    ...
    <div className="grid grid-cols-12 lg:grid-rows-6 gap-6 min-h-[700px] lg:h-[calc(100vh-200px)]">
      ...
      <div className="col-span-12 lg:col-span-4 lg:row-span-3 bg-slate-900/50 border border-slate-900 rounded-3xl p-6 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between mb-6 shrink-0">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Formal Definition</h3>
          <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded text-indigo-400 font-mono">DFA_SPEC</span>
        </div>
        ...
        <div className="space-y-6 flex-grow overflow-y-auto pr-2 custom-scrollbar">
          <div>
            ...
            <label className="block text-[10px] font-bold text-slate-600 uppercase mb-3 tracking-widest">Test String</label>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/50 flex gap-3">
          <button onClick={run} disabled={isFinished} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-full flex items-center justify-center gap-2 text-xs font-bold shadow-lg shadow-indigo-600/20 disabled:opacity-30 transition-all">
            <Play className="w-4 h-4 fill-white" /> RUN
          </button>
          <button onClick={step} disabled={isFinished} className="w-14 h-14 bg-slate-800 hover:bg-slate-700 text-white rounded-full flex items-center justify-center disabled:opacity-30 transition-all border border-slate-700">
            <StepForward className="w-5 h-5" />
          </button>
          <button onClick={reset} className="w-14 h-14 bg-slate-800 hover:bg-slate-700 text-white rounded-full flex items-center justify-center transition-all border border-slate-700">
      ...
      <div className="col-span-12 lg:col-span-8 lg:row-span-4 bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden relative shadow-2xl">
        <div className="absolute top-6 left-6 z-10">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">State Visualization</h3>
        </div>
        <div className="flex items-center justify-center h-full">
          {isValidRegex ? (
            ...
            <div className="flex flex-col items-center gap-4 text-slate-700">
              ...
              <p className="text-xs font-bold uppercase tracking-widest">Compiler Error: Build Failed</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="col-span-12 lg:col-span-4 lg:row-span-3 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 flex flex-col overflow-hidden">
        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-6 shrink-0">Transition Table</h3>
          ...
          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 font-mono text-[11px] leading-relaxed">
            ...
            <div className="mt-4 space-y-1">
              {inputString.split('').map((char, idx) => (
                <p key={idx} className={`flex justify-between ${idx === currentIndex ? 'text-white font-bold' : idx < currentIndex ? 'text-indigo-500' : 'text-slate-700'}`}>
                  <span>Step {idx + 1}: Read '{char}'</span>
                  <span>{idx < currentIndex ? '-> q_resolved' : idx === currentIndex ? '-> processing' : ''}</span>
                </p>
              ))}
            </div>
          </div>
        ...
        <div className="mt-6">
          <div className={`p-4 rounded-2xl border transition-all duration-300 flex items-center justify-between ${isFinished ? (isAccepted ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-rose-500/10 border-rose-500/30') : 'bg-slate-950 border-slate-800'}`}>
            <div>
              <span className="text-[10px] text-slate-600 font-bold uppercase block mb-1">Compiler Result</span>
              <span className={`text-sm font-black tracking-widest ${isFinished ? (isAccepted ? 'text-emerald-400' : 'text-rose-400') : 'text-slate-500'}`}>
                {isFinished ? (isAccepted ? 'ACCEPTED' : 'REJECTED') : 'IDLE_STATE'}
              </span>
            </div>
            {isFinished && (
              isAccepted ? <CheckCircle2 className="w-6 h-6 text-emerald-400 animate-bounce" /> : <AlertCircle className="w-6 h-6 text-rose-400 animate-pulse" />
            )}
          </div>
        </div>
      </div>
      ...
      <div className="col-span-12 lg:col-span-8 lg:row-span-2 bg-slate-900/50 border border-slate-900 rounded-3xl p-6 flex items-center gap-6">
        ...
        <div className="flex-grow">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Automata Regular Expression</h3>
