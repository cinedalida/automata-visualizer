...
import { Send, Terminal, ClipboardList, CheckCircle2, AlertCircle, ChevronRight, Hash } from 'lucide-react';
...
export default function CFGSimulatorUI() {
  ...
  const [selectedCfgKey, setSelectedCfgKey] = useState<keyof typeof CFG_EXAMPLES>('cfg1');
    ...
    <div className="grid grid-cols-12 lg:grid-rows-6 gap-6 min-h-[700px] lg:h-[calc(100vh-200px)]">
      ...
      <div className="col-span-12 lg:col-span-4 lg:row-span-3 bg-slate-900/50 border border-slate-900 rounded-3xl p-6 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between mb-6 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-white uppercase text-xs tracking-widest">CFG Compiler</h2>
          </div>
          <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded text-emerald-400 font-mono">CFG_PARSER</span>
        </div>
        ...
        <div className="space-y-6 flex-grow overflow-y-auto pr-2 custom-scrollbar">
          <div>
            ...
            <label className="block text-[10px] font-bold text-slate-600 uppercase mb-3 tracking-widest">Source String</label>
          ...
          <div className={`p-4 rounded-2xl border transition-all duration-300 flex items-center justify-between ${result?.isAccepted ? 'bg-emerald-500/10 border-emerald-500/30' : result ? 'bg-rose-500/10 border-rose-500/30' : 'bg-slate-950 border-slate-800'}`}>
            <div>
              <span className="text-[10px] text-slate-600 font-bold uppercase block mb-1">Status</span>
              <span className={`text-sm font-black tracking-widest ${result ? (result.isAccepted ? 'text-emerald-400' : 'text-rose-400') : 'text-slate-500'}`}>
                {isValidating ? 'PARSING...' : (result ? (result.isAccepted ? 'VALID_GRAMMAR' : 'REJECTED') : 'IDLE')}
              </span>
            </div>
            {result && (
              result.isAccepted ? <CheckCircle2 className="w-5 h-5 text-emerald-400" /> : <AlertCircle className="w-5 h-5 text-rose-400" />
            )}
          </div>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/50">
          <p className="text-[10px] text-slate-600 leading-relaxed font-mono italic">
            [SYS] Performing BFS sentential form expansion... Max depth 1000 nodes.
          </p>
        </div>
      </div>

      {/* Grammar Rules Bento Box */}
      <div className="col-span-12 lg:col-span-8 lg:row-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
        {/* Background Grid */}
        <div className="absolute inset-0 opacity-[0.02] pointer-events-none z-0" style={{ backgroundImage: 'radial-gradient(#fff 1px, transparent 1px)', backgroundSize: '16px 16px' }}></div>
        
        <div className="relative z-10">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-6">Formal Grammar (G)</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {isValidRegex ? Object.entries(cfg.productions).map(([v, prods]) => (
              <div key={v} className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50">
                <span className="text-[10px] text-emerald-500 font-bold block mb-1">{v}</span>
                <div className="font-mono text-sm text-slate-300">
                  {prods.join(' | ')}
                </div>
              </div>
            )) : (
              <div className="col-span-4 text-center py-4 text-slate-700 font-mono text-[10px] uppercase">
      ...
      <div className="col-span-12 lg:col-span-8 lg:row-span-4 bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col overflow-hidden shadow-xl">
        <div className="flex items-center justify-between mb-8 shrink-0">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Derivation Steps</h3>
        ...
        <div className="flex-grow overflow-y-auto pr-2 custom-scrollbar">
          {isValidating ? (
            <div className="flex flex-col items-center justify-center h-full gap-4">
              ...
              />
              <span className="text-slate-500 animate-pulse text-xs uppercase tracking-widest font-mono">Tracing derivation...</span>
            </div>
          ) : result?.isAccepted ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              {result.steps.map((step, idx) => (
                ...
                >
                  ...
                  <div className={`font-mono text-sm px-3 py-1 rounded bg-slate-950/50 border border-slate-800/50 ${idx === result.steps.length - 1 ? 'text-emerald-400 font-bold border-emerald-500/20' : 'text-slate-400'}`}>
                    {step === '' ? 'ε' : step}
                  </div>
                </motion.div>
              ))}
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4 opacity-50">
              ...
              <span className="text-rose-400 text-xs font-bold uppercase tracking-widest">Syntax Analysis Error</span>
              <p className="text-slate-600 text-[10px] max-w-[250px] text-center font-mono uppercase tracking-tighter mt-2">
                "Compiler could not resolve the token sequence for the defined grammar productions."
              </p>
            </div>
          ) : (
      ...
      <div className="col-span-12 lg:col-span-4 lg:row-span-3 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 flex flex-col overflow-hidden">
        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-6 shrink-0">Parsing Summary</h3>
        <div className="space-y-4 flex-grow overflow-y-auto pr-2 custom-scrollbar">
          <div className="bg-emerald-500/5 p-4 rounded-2xl border border-emerald-500/10">
            <h4 className="text-[10px] font-bold text-emerald-500 uppercase mb-2">Grammar Type</h4>
            <p className="text-xs text-slate-400 leading-relaxed">
              Detected Context-Free (Type 2). Ready for PDA mapping.
            </p>
          </div>
          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800">
            <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-2">Automata Parity</h4>
            <p className="text-xs text-slate-400 leading-relaxed">
        ...
        <div className="mt-auto pt-6 border-t border-slate-800/50 flex items-center justify-between">
          <span className="text-[10px] text-slate-700 font-mono">GEN_MODE: BFS_SENTENTIAL</span>
