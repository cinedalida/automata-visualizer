...
const D3Graph: React.FC<D3GraphProps> = ({
  ...
  useEffect(() => {
    ...
    const link = g.append('g')
      ...
      .attr('fill', 'none')
      .attr('marker-end', (d: any) => d.isActive ? 'url(#arrowhead-active)' : 'url(#arrowhead)');
      ...
      .each(function(d: any) {
        ...
        d.labels.forEach((label: any, i: number) => {
          el.append('tspan')
            .attr('x', 0)
            .attr('dy', i === 0 ? 0 : '1.1em')
            .attr('fill', label.active ? '#818cf8' : (d.isActive ? '#334155' : '#64748b'))
    ...
    node.append('circle')
      .attr('r', 20)
      .attr('fill', (d: any) => d.id === activeStateId ? '#6366f1' : '#0f172a')
      .attr('stroke', (d: any) => {
    ...
    node.filter((d: any) => d.isAccept).append('circle')
      .attr('r', 16)
      .attr('fill', 'none')
      .attr('stroke', '#10b981')
      .attr('stroke-width', 1);
    ...
    node.append('text')
      ...
      .attr('text-anchor', 'middle')
      .attr('dy', 5)
      .attr('fill', (d: any) => d.id === activeStateId ? '#ffffff' : '#94a3b8')
      .attr('font-size', '12px')
      .attr('font-weight', 'bold');

    // Start arrow
    node.filter((d: any) => d.isStart).append('path')
    ...
    const yMax = (d3.max(nodes, (d: any) => d.y) as unknown as number) ?? height;
    ...
    const scale = Math.min(0.9, width / (graphWidth + 100), height / (graphHeight + 100));
    ...
    link.attr('d', (d: any) => {
      ...
      const hasReverse = links.some(l => l.source.id === d.target.id && l.target.id === d.source.id);
      if (hasReverse) {
